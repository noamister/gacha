...
#ボタンの値を持ってきて、且つｄｂからも持ってきて、そして総額、差額、おつりも計算したい、最後はhtmlに反映
@app.route('/result',methods="GET")
def result(charge):
    charge = "charge"
    #buttonの値を持ってくる→「〇〇〇円で買えるお菓子は」
     
    #dbから値を持ってき、総額と予算との差額を計算→「おかしのねだんは/おつりは」
    
    gacha = cur.execute('insert into purchase (item, price, img) values (select item, price, img where items) ;')
    if cur.execute('select  sum(price) where purchase ;') < charge*100 or null #最初の操作時、空なのでNULL
        return gacha()
    else:
        #ガチャの商品、価格、画像、個数を表示
        return cur.execute('select item, price, img, count(item)  where purchase order by item')
        #合計金額算出
               cur.execute('select sum(price) as cost where purchase')
        
    #ここで持ってきたデータをｈｔｍｌの特定部に反映
    charge="charge"*100
    cost ="cost"
    ...
    #     (SELECT ABS(RANDOM() % (SELECT MAX(rowid) FROM items))
    #    UNION ALL SELECT ABS(RANDOM() % (SELECT MAX(rowid) FROM items)) FROM random_id)
